...
describe('CustomSelect — invalid prop (red boundary on selects)', () => {
  ...
  it('renders the fork label caption without altering the trigger accessible name', () => {
    render(<CustomSelect ariaLabel="pick" label="Agent Profile" value="" onChange={() => {}} options={OPTIONS} />)
    expect(screen.getByText('Agent Profile')).toBeInTheDocument()
    // Caption is a plain div, not an associated label: the trigger keeps its aria-label name.
